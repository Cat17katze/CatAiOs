...
function install(getAPM,getSKY)
  ...
  if getAPM then
    bar((1/items)*100,"Downloading files","APM2")
    eta(items,2,delay)
    wget("https://raw.githubusercontent.com/TheAio/CC-APM/main/APM2","APM2")
    bar((2/items)*100,"Downloading files","APMs")
    eta(items,3,delay)
    wget("https://raw.githubusercontent.com/TheAio/CC-APM/main/APMs","APMs")
    bar((3/items)*100,"Fetching APM2 files","FTP")
    eta(items,4,delay)
    shell.run("APM2 install FTP")
    if getSKY then
      bar((4/items)*100,"Fetching APM2 files","skynet")
      eta(items,5,delay)
